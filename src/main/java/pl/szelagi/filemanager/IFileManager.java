...
public interface IFileManager {
    ...
    @NotNull
    File getHeadDirectory();
    default IFileManager getPreviousDirectory() throws BeyondHeadDirectoryException {
        if (getCurrentDirectory().equals(getHeadDirectory())) throw new BeyondHeadDirectoryException(getCurrentDirectory().getPath());
        var parentDirectory = new File(getCurrentDirectory().getParent());
        if (!parentDirectory.exists() || !parentDirectory.isDirectory()) throw new DirectoryNotFoundException(parentDirectory.getPath());
    ...
    default IFileManager getDirectory(String name) throws DirectoryNotFoundException {
        ...
        var newHeadDirectory = getHeadDirectory();
        if (!newCurrentDirectory.exists() || !newCurrentDirectory.isDirectory()) throw new DirectoryNotFoundException(newCurrentDirectory.getPath());
        return new IFileManager() {
            @Override
            public @NotNull File getCurrentDirectory() {
    ...
    default boolean existsDirectory(String name) {
        if (!getCurrentDirectory().exists() || !getCurrentDirectory().isDirectory()) return false;
    ...
    default void tryMakeHeadDirectory() throws InvalidFileTypeException {
        if (getHeadDirectory().exists() && getHeadDirectory().isFile()) throw new InvalidFileTypeException(getHeadDirectory().getPath() + "type is file, instead of directory!");
