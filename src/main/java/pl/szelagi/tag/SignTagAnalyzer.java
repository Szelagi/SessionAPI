...
public class SignTagAnalyzer {
    private final static ArrayList<Material> SIGN_MATERIALS = new ArrayList<>(Arrays.asList(
            Material.OAK_SIGN,
    ...
    public static SignTagData process(@NotNull ISpatial spatial,
        ...
        for (var b : blocks) {
            if (!SIGN_MATERIALS.contains(b.getType())) continue;
            ...
            if (mainLine.startsWith(prefix)) {
                ...
                element = new SignTag(name, relativeLocation, args);
                data.add(element);
            }
