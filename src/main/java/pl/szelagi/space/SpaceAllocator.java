...
public class SpaceAllocator {
    ...
    private static boolean setTimeLock(@NotNull ArrayList<SpaceAllocate> allocates, int slot, long lockMillis) {
        ...
        if (spaceAllocate == null) return false;
    ...
    private static boolean isTimeBlocked(@NotNull ArrayList<SpaceAllocate> allocates, int slot) {
        var spaceAllocate = getSpaceAllocate(allocates, slot);
        if (spaceAllocate == null) return false;
        if (!spaceAllocate.isPutOnTimeLock()) return false;
    ...
    private static boolean deleteAllocate(SpaceAllocate spaceAllocate) {
        ...
        if (allocates == null) return false;
    ...
    public static Space allocate(World world) {
        ...
        while (isReservedSlot(allocates, slot) || isTimeBlocked(allocates, slot)) slot++;
    ...
    public static void deallocate(Space space) {
        ...
        if (allocates == null) throw new DeallocateException("allocates ArrayList is not initialized", space);
        if (allocates.isEmpty()) throw new DeallocateException("allocates ArrayList is empty", space);
        var spaceAllocate = allocates.stream().filter(sa -> sa.getSlot() == space.getSlot()).findFirst().orElse(null);
        if (spaceAllocate == null) throw new DeallocateException("allocates ArrayList do not exists element", space);
